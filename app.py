...
st.set_page_config(page_title="Genomic AI Agent", layout="wide")
st.title("🧬 Genomic AI Agent – Just Ask")
...
with st.sidebar:
    ...
    manual_agents = st.multiselect("Select agents to run (empty = auto)", 
...
if st.button("Run", type="primary"):
    ...
    if files.get("vcf_csv") and Path(files["vcf_csv"]).exists():
        ...
        st.download_button("Download VCF CSV", data=df.to_csv(index=False),
    ...
    if files.get("joined_csv") and Path(files["joined_csv"]).exists():
        ...
        st.download_button("Download Joined CSV", data=df_joined.to_csv(index=False),
                           file_name=Path(files["joined_csv"]).name)

    # Show plot if it exists, else show the generated code (if any)
